	...
	public void testReport() throws Exception {
		...
		for (Iterator<DataSetRow> itr = data.getDataSets().get("Antenatal Risks").iterator(); itr.hasNext();) {
			...
			Cohort prenatalMalariaChloroquine = (Cohort) row.getColumnValue(
			    "Prenatal visit + malaria test positive + Chloroqine co." + AntenatalReportManager.risksCol1);
			assertNotNull(prenatalMalariaChloroquine);
			assertEquals(1, prenatalMalariaChloroquine.getSize());
			
			Cohort prenatalMUAC = (Cohort) row.getColumnValue("Prenatal + MUAC =<21cm." + AntenatalReportManager.risksCol1);
			assertNotNull(prenatalMUAC);
			assertEquals(1, prenatalMUAC.getSize());
			
			Cohort otherWomenIronFolate = (Cohort) row
			        .getColumnValue("Women + fer folate co prescribed." + AntenatalReportManager.risksCol1);
