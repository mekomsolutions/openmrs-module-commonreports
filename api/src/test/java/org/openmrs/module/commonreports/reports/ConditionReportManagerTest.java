...
public class ConditionReportManagerTest extends BaseModuleContextSensitiveMysqlBackedTest {
	
	public ConditionReportManagerTest() throws SQLException {
		super();
	...
	@Autowired
	@Qualifier("conditionsReportManager")
		...
		for (DataSet ds : data.getDataSets().values()) {
			...
			for (Iterator<DataSetRow> itr = ds.iterator(); itr.hasNext();) {
				...
				if (rowNumber == 1) {
					...
					assertEquals(row1columnValuePairs.get("birthdate"), row.getColumnValue("birthdate"));
					...
					assertEquals(row1columnValuePairs.get("date_created"), row.getColumnValue("date_created"));
				...
				if (rowNumber == 7) {
					...
					assertEquals(row7columnValuePairs.get("birthdate"), row.getColumnValue("birthdate"));
